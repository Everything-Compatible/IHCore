...
| **事件回调** (Callbacks) | OnSessionClosed / OnBuildingReady / OnUnitDestroyed / OnUnderAttack / OnChatMessage / OnBeacon，会话级自动推送 |
...
| `GetCurrentTime` | 当前游戏已经过时间（帧数/秒数） |
| `GetPlayerViewRoute` | 指定时间范围玩家屏幕中心轨迹（0.5s 采样） |
...
| **回调机制** | OnSessionClosed / OnBuildingReady / OnUnitDestroyed / OnUnderAttack / OnChatMessage / OnBeacon 通过 `PostAsyncRemoteCall` 推送 |
