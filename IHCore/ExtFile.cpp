...
bool ExtFileClass::ReadLabel(std::string& Str)const//´ý²âÊÔ£¡£¡
{
    const size_t MaxSize = 65536;
    static char Buf[MaxSize];
    size_t Cur = 0;
...
template<typename T>
bool ExtFileClass::WriteVector(const std::vector<T>& Data)const//´ý²âÊÔ£¡£¡
{
    if (!WriteData((int64_t)Data.size()))return false;
    return Write(Data.data(), sizeof(T), Data.size());
}
template<typename T>
bool ExtFileClass::ReadVector(std::vector<T>& Data)const//´ý²âÊÔ£¡£¡
{
    int64_t Size;
...
int ExtFileClass::WriteVector(const std::vector<std::string>& Data)const//´ý²âÊÔ£¡£¡
{
...
int ExtFileClass::ReadVector(std::vector<std::string>& Data)const//´ý²âÊÔ£¡£¡
{
    int64_t Size;
...
template<typename T>
int ExtFileClass::WriteVector(const std::vector<T>& Data, const std::function<bool(const ExtFileClass&, const T&)>& Proc)const//´ý²âÊÔ£¡£¡
{
    if (!WriteData((int64_t)Data.size()))return 0;
    int Ret = 0;
    for (const auto& d : Data)
...
template<typename T>
int ExtFileClass::WriteVector(std::vector<T>& Data, const std::function<bool(const ExtFileClass&, T&)>& Proc)const//´ý²âÊÔ£¡£¡
{
    if (!WriteData((int64_t)Data.size()))return 0;
    int Ret = 0;
    for (auto& d : Data)
...
template<typename T>
int ExtFileClass::ReadVector(std::vector<T>& Data, const std::function<bool(const ExtFileClass&, T&)>& Proc)const//´ý²âÊÔ£¡£¡
{
    int64_t Size;
    if (!ReadData(Size))return 0;
    Data.resize((size_t)Size);
