...
R"({{
  ...
  "Context": {}
		...
		, ~Source, ~Component, ~Method, Version, CallID, CtxString);
}
...
namespace RemoteComponentManager
{
	...
	*/
	...
	void RegisterComponent(RemoteComponentNameType&& Com, const InitInput& Input)
	{
		...
		if (Ptr->Connect())
		{
			if (Ptr->Register(Input))RemoteComponents[Ptr->GetName()] = std::move(Ptr);
			else Ptr->Disconnect();
		}

		RemoteComponents[Ptr->GetName()] = std::move(Ptr);
	}
	...
	*/
	...
	{
		RemoteCallReturnInfo Ret;
		auto ReturnInfo = [Info, &Ret]() -> RemoteCallRecvInfo
			{
				if (Ret.ErrorMessage)
				{
					auto& ErrorStr = *Ret.ErrorMessage;
					Debug::Log("[EC] RCM : %s\n", ErrorStr.c_str());
					Debug::Log("[EC] Request : %s", Info.GenerateText().c_str());
				}
	...
	*/
	...
	void ProcessAsyncLocalTask()
	{
		...
		{
			AsyncLocalRecv.Push(RespondLocalCall(Post.Forward()));
		}
	}
}
