...
struct ForwardNode
{
	ForwardNode* PreviousEntry;
	ForwardNode() : PreviousEntry(nullptr) { }
	T* Prev() { return (T*)PreviousEntry; }
	operator T* ()
	{
		return (T*)this;
	}
...
struct ForwardList
{
	ForwardNode<T>* Header;
	void Destroy()
	{
		for (auto i = Header; i; i = i->PreviousEntry)
		{
			GameDelete(i);
		}
		Header = nullptr;
	}
	void Push(ForwardNode<T>* Node)
	{
		Node->PreviousEntry = Header;
		Header = Node;
	}
	bool Empty()
	{
