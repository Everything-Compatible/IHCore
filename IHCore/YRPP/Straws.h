...
public:
	...
	void Insert_From(Straw& pipe)
	{
		auto orig = this->ChainFrom;
		this->ChainFrom = &pipe;
		pipe.ChainFrom = orig;
	}

	void Insert_To(Straw& pipe)
	{
		auto orig = this->ChainTo;
		this->ChainTo = &pipe;
		pipe.ChainTo = orig;
	}
...
struct CacheStraw : public Straw
{
	MemoryBuffer Buffer;
	int BufferPos;
	int BufferedBytes;
