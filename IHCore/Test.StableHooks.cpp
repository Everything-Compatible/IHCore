...
/*
DEFINE_HOOK(0x52CF7C, LoadRuleFiles, 6)
{
	REF_STACK(DynamicVectorClass<CCINIClass*>, RuleInis, -0x308);
	//REF_STACK(CCFileClass, RulesMD, -0x2F0);
	//GET(CCINIClass*, pRule, ESI);
	//fg(pRule);
	//GTC = reinterpret_cast<CCFileClass*>(&RulesMD);
	R->ECX(RuleInis.Capacity);
	__asm {cmp eax, ecx};
	return 0x52CF82;
}

BOOL CCINIClass_ReadFile_Ex(CCINIClass* pINI, CCFileClass* pFile)
{
	//pFile
	::MessageBoxA(NULL, std::to_string(pFile).c_str(), "TCC", MB_OK);
	::MessageBoxA(NULL, std::string(pFile->GetFileName()).c_str(), "TCC", MB_OK);
	//::MessageBoxA(NULL, std::to_string(pFile->GetFileSize()).c_str(), "TCD", MB_OK);
	//::MessageBoxA(NULL, "SSEF", "TCC", MB_OK);
	
	return INIClass_ReadFile(pINI, pFile);
}*/
/*
DEFINE_HOOK(0x525A60, CCINIClass_ReadFile, 5)
{
	GET(CCINIClass*, pINI, ECX);
	GET_STACK(FileStraw*, pFile, 0x4);
	R->EAX(CCINIClass_ReadFile_Ex(pINI, pFile->File));
	//::MessageBoxA(NULL, "11u", "tdfh2", MB_OK);
	return 0x525D85;
}*/
...
/*
DEFINE_HOOK(0x47AD41, MakeCDFilePath, 5)
{
	CDExt_Instance.OnRemakeCDPath();
	return 0;
}


DEFINE_HOOK(0x47AE21, CDFileClass_SetFileName_1, 5)
{
	GET(CDFileClass*, pFile, EBX);
	GET_STACK(const char*, pFileName, 0x4);
	if (CDExt_Instance.SearchFileBeforeGameDir(pFile, pFileName))
	{
		R->EAX(pFile->GetFileName());
		return 0x47AEF0;
	}
	else return 0;
}
