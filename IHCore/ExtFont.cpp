...
{
    const static short ColorSet[20] =
    {
        ...
        (short)0xDEA0, (short)0xE6BA, (short)0xCE59, (short)0x41C7,
    };
    bool Styled{ false };
    if (ws[0] == L'§' && ws[1] == L'§')//OK
    {
...
#undef u
                {
                    ...
                    else
                    {
                        ...
                        cd.Col16 = ((((short)c.R) >> 3 << 11) | (((short)c.G) >> 2 << 5) | ((short)c.B) >> 3);
                    }
                    ws += 4;
                    Styled = true;
                }
            ...
            else if (iswdigit(ws[1]))
            {
                cd.HasColor = true;
                cd.Bold = cd.StrikeThrough = cd.Underline = false;
                cd.Col16 = ColorSet[ws[1] - L'0'];
                ws += 2;
                Styled = true;
            }
            else if (ws[1]>=L'a' && ws[1]<=L'j')
            {
                cd.HasColor = true;
                cd.Bold = cd.StrikeThrough = cd.Underline = false;
                cd.Col16 = ColorSet[ws[1] - L'a' + 10];
                ws += 2;
                Styled = true;
            }
            else if (iswalpha(ws[1]))
            {
                ws += 2;
                Styled = true;
            }
