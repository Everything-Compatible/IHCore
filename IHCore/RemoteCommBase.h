...
concept RemoteCommType = requires(T Comm, std::u8string& U)
{
	...
	{ Comm.BaseConnect(std::u8string_view{}, std::u8string_view{}, U, bool{}, int{}) } -> std::same_as<bool>;
...
public:
	...
	bool Connect(std::u8string_view RegName, std::u8string_view CmdLine, std::u8string& Location, bool KeepAliveOnProcessExit, int TimeOut);
	void Disconnect();
...
template <typename T>
bool RemoteComponentBase<T>::Connect(std::u8string_view RegName, std::u8string_view CmdLine, std::u8string& Location, bool KeepAliveOnProcessExit, int TimeOut)
{
	IPC_Log("[EC] RCB : Connecting Base \"%s\" Initial Location \"%s\"\n", conv RegName.data(), conv Location.c_str());
	if (!inst()->BaseConnect(RegName, CmdLine, Location, KeepAliveOnProcessExit, TimeOut))return false;
